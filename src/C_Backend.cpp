  ...
  void generateExpression(ostream& c, Expression* expr)
  {
    ...
    else if(CallExpr* call = dynamic_cast<CallExpr*>(expr))
    {
      if(auto subrExpr = dynamic_cast<SubroutineExpr*>(call->callable))
      {
        ...
        {
          if(subrExpr->thisObject->assignable())
          {
            //"this" is persistent, so can take its address directly to get "this" ptr
            c << '&';
            generateExpression(c, subrExpr->thisObject);
          }
          else
          {
            //need to get thisObject in a temp, then take address (use stmt expr)
            string thisTemp = getIdentifier();
            c << "({" << types[subrExpr->thisObject->type] << ' ' << thisTemp << " = ";
            generateExpression(c, subrExpr->thisObject);
            c << "; &" << thisTemp << "})";
          }
        }
      ...
      else
      {
        ...
        if(auto sm = dynamic_cast<StructMem*>(call->callable))
        {
          ...
          {
            //evaluate base and assign to a temp, then take temp's address
            string baseTemp = getIdentifier();
            c << "({" << types[sm->base->type] << " " << baseTemp << " = ";
            generateExpression(c, sm->base);
            c << "; ";
        ...
        {
          //just evaluate the expression which is a function ptr
